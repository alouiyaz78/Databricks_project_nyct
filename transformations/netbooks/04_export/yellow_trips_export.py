...
df = spark.read.table("nyctaxi.01_silver.yellow_trips_enriched").filter(f"tpep_pickup_datetime > '{two_months_ago_start}'")
...
df.write.\
    option("path", "abfss://nyctaxi-yellow@nyctaxistorage78.dfs.core.windows.net/yellow_trips_export/").\
    format("json").\
    mode("append").\
