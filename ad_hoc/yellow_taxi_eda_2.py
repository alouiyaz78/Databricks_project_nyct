...
spark.read.table("nyctaxi.`02_bronze`.yellow_trips_raw").\
...
spark.read.table("nyctaxi.`01_silver`.yellow_trips_cleansed").\
    groupBy(date_format("tpep_pickup_datetime", "yyyy-MM").alias("year_month")).\
    agg(count("*").alias("total_records")).\
    orderBy("year_month").display()
...
spark.read.table("nyctaxi.`01_silver`.yellow_trips_enriched").\
    groupBy(date_format("tpep_pickup_datetime", "yyyy-MM").alias("year_month")).\
    agg(count("*").alias("total_records")).\
    orderBy("year_month").display()
...
spark.read.table("nyctaxi.`03_gold`.daily_trips_summary").\
